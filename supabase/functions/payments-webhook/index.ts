...
// Plan tier mapping based on Stripe price IDs or amounts
function determinePlanTier(subscription: any): string {
  const amount = subscription.items?.data[0]?.price?.unit_amount || 0;
  const priceId = subscription.items?.data[0]?.price?.id;

  // Map based on price ID first (more reliable)
  ...
  // Fallback to amount-based mapping (in cents)
  if (amount === 0) return "free";
  if (amount <= 1999) return "basic"; // $19.99 or less
  if (amount <= 4999) return "pro"; // $49.99 or less
  return "enterprise"; // Above $49.99
}
...
async function handleSubscriptionDeleted(supabaseClient: any, event: any) {
  ...
  try {
    // Update subscription status and set plan to free
